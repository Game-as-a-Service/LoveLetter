...
class ExampleApiTests(unittest.TestCase):
    ...
    def test_http_get_read_item_with_parameters(self):
        response = self.t.get(
            "/items_with_params/give_me_10_secs?needy=c8763&limit=10"
        ).json()
        self.assertEqual(
            dict(item_id="give_me_10_secs", limit=10, needy="c8763", skip=0), response
        )
    ...
    def test_http_post(self):
        response = self.t.post(
            "/items",
            json=dict(
                name="say my name", price=3.14, tax=0.1, description="breaking bad"
            ),
        ).json()
        self.assertEqual(
            dict(name="say my name", price=3.14, tax=0.1, description="breaking bad"),
            response,
        )
    ...
    def test_http_post_with_response_model(self):
        ...
        self.assertEqual(
            dict(username="9527", email="3345678@foobarbar.com", full_name="full_name"),
            response,
        )
